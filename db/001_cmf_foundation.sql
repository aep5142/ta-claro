-- Unified bank credit-card operations ETL foundation.
-- This migration is intentionally separate from the existing UF tables.

create table if not exists public.bank_credit_card_ops_registry (
    dataset_code text primary key,
    operation_type text not null,
    transaction_count_source_tag text not null,
    nominal_volume_source_tag text not null,
...
create table if not exists public.bank_credit_card_ops_sync_state (
    dataset_code text primary key references public.bank_credit_card_ops_registry(dataset_code),
...
create table if not exists public.bank_credit_card_ops_raw (
    operation_type text not null,
    dataset_code text not null references public.bank_credit_card_ops_registry(dataset_code),
...
create table if not exists public.bank_credit_card_ops_curated (
    ...
    source_dataset_code text not null references public.bank_credit_card_ops_registry(dataset_code),
    updated_at timestamptz not null default now(),
...
insert into public.bank_credit_card_ops_registry (
    dataset_code,
    operation_type,
