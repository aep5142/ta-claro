...
def test_cmf_cards_cleanup_sql_defines_renamed_tables_and_metrics_surface():
    ...
    assert "create table if not exists public.bank_credit_card_purchases_metrics" in sql
    assert "average_ticket_uf" in sql
    assert "create or replace view public.bank_credit_card_purchases" in sql
